...
def main():
    ...
    if st.session_state['authentication_status']:
        ...
        if namespace_list:
            ...
            namespace = st.selectbox("Select Location/Site", namespace_list, index=ns_index)
            if namespace:
            ...
            if sw:
                ...
                filtered_data = [item for item in intf_list_response.json() if "Ethernet" in item["ifname"]]
                ...
                filtered_interfaces = [iface for iface in unsorted_intf_list if re.search(r"thernet(\d)/0/(\d{1,2})$", iface)]
            ...
            vlan_df['humanTimestamp'] = vlan_df['timestamp'].apply(utils.unix_to_human_local)
            ...
            st.write(sorted_df[["namespace", "hostname", "Vlan", "vlanName", "state", "interfaces"]])
            ...
            if new_vlan_picked:
                newvlan_df = sorted_df[sorted_df['pick_list'] == new_vlan_picked].reset_index()
                ...
                st.warning(f"Please confirm Vlan interface change on interface {intf} from vlan {intf_vlan} {vlan_name} to NEW vlan {new_vlan_picked}")
                ...
                if reason and confirm == "Confirm":  # and stp_check:
                    ...
                    with st.form(key="CHANGE_PORT_VLAN"):
                        ...
                        if change_vlan:
                            ...
                            st.write(f"Sending the following configuration update to {sw} {intf} and saving configuration to device:")
                            ...
                            show_int_status = conn.send_command(f"show interface status", use_textfsm=True)
                            ...
                            if conn:
                                ...
                                st.info(f"Executing configuration commands to move interface into vlan {new_vlan}...")
                                ...
                                st.info("IP Addressing Method not determined so no further testing is possible. Please verify system is functional.")
                                conn.disconnect()
