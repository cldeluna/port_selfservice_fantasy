...
def add_new_user(config: Dict, username: str, email: str, name: str, password: str, role: str = 'user') -> bool:
...
def main():
    ...
    with st.form('change_password'):
        ...
        if submit_change:
            if not verify_password(current_password, config['credentials']['usernames'][current_user]['password']):
                ...
                config['credentials']['usernames'][current_user]['password'] = hash_password(new_password)
                save_config(config)
    ...
    if admin_access:
        ...
        with st.form('add_user'):
            ...
            if submit_new_user:
                if not all([new_username, new_user_email, new_user_name, new_user_password]):
                    ...
                    if add_new_user(config, new_username, new_user_email, new_user_name, new_user_password, new_user_role):
        ...
        with st.form('admin_set_password'):
            ...
            new_admin_password = st.text_input('New Password', type='password', key='admin_new')
            confirm_admin_password = st.text_input('Confirm Password', type='password', key='admin_confirm')
            user_role = st.selectbox('User Role', ['user', 'admin'], 
                                   index=0 if config['credentials']['usernames'][selected_user].get('role') != 'admin' else 1)
            ...
            if submit_admin:
                if new_admin_password:
                    if new_admin_password != confirm_admin_password:
                        ...
                        config['credentials']['usernames'][selected_user]['password'] = hash_password(new_admin_password)
